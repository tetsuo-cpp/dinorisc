...
def compile_sample(source_file):
...
def compiled_elfs():
    """Compile each unique source file once and cache the ELF paths."""
    elfs = {}
    sources = {case[0] for case in TEST_CASES}
    for source in sources:
        elfs[source] = compile_sample(source)
    yield elfs
    for path in elfs.values():
        os.unlink(path)
...
def test_dinorisc_execution(
    compiled_elfs, source_file, function_name, args, expected_return
):
    """Test that dinorisc correctly executes RISC-V binaries and produces expected results."""
    if not DINORISC_BIN.exists():
        pytest.skip("dinorisc binary not found")

    elf_path = compiled_elfs[source_file]
    cmd = [str(DINORISC_BIN), elf_path, function_name] + [str(a) for a in args]
    ...
    assert int(match.group(1)) == expected_return
